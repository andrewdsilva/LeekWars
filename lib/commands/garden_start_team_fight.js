...
function select_enemy_auto(compositionId, teamId, choices)
{
    ...
    launch_fight(compositionId, teamId, chosen_composition.id);
}

function select_enemy_manuel(compositionId, teamId, choices)
{
    var i = 1;
    ...
    rl.on('line', function(line) {
        if (line > 0 && line < i){
            launch_fight(compositionId, teamId, choices[line-1].id);
            //OK, we've finished
            rl.close();
        }
        else {
...
function launch_fight(compositionId, teamId, enemyId)
{
        ...
        services.get_garden(function(data){
            Object.keys(data.garden.my_compositions).forEach(function(key) {
                ...
                if (value.name == argv.name)
                {
                    ...
                    if (argv.auto)
                    {
                        select_enemy_auto(compositionId, teamId, oponents);
                    }
                    else {
                        select_enemy_manuel(compositionId, teamId, oponents);
                    }
