...
function select_enemy_auto(choices)
{
    ...
    launch_fight(chosen_farmer.id);
}

function select_enemy_manuel(choices)
{
    var i = 1;
    ...
    rl.on('line', function(line) {
        if (line > 0 && line < i){
            launch_fight(choices[line-1].id);
            //OK, we've finished
            rl.close();
        }
        else {
...
function garden_start_farmer_fight(argv)
{
    services.login(function(data) {
        services.get_garden(function(data){
            ...
            {
                select_enemy_auto(oponents);
            }
            else {
                select_enemy_manuel(oponents);
            }
